...
/**
 * E-mail po schválení veľkoobchodného účtu.
 * Zatiaľ len template, odosielanie napojíme neskôr.
 */
export function buildWholesaleApprovedEmail(
  vars: WholesaleApprovedEmailVars,
): EmailTemplate {
  ...
  const loginUrl =
    vars.loginUrl ?? absolutize(siteUrl, "/prihlasenie/velkoobchod");
  ...
  const subject = firstName
    ? `${firstName}, váš veľkoobchodný účet je schválený`
    : "Váš veľkoobchodný účet je schválený";

  const preheader =
    "Účet je aktívny. Môžete sa prihlásiť a začať nakupovať.";

  const text = [
    greeting,
    "",
    "radi vás informujeme, že sme vašu žiadosť o veľkoobchodný účet schválili.",
    "",
    company
      ? `Účet pre firmu ${company} je aktívny.`
      : "Váš veľkoobchodný účet je aktívny.",
    "",
    "Odteraz sa môžete prihlásiť a využívať veľkoobchodné ceny a výhody.",
    "",
          ...
          <tr>
            <td class="email-pad" style="padding: 16px 36px 8px 36px; font-family:${EMAIL_FONT_SANS}; font-size:16px; line-height:1.65; color:${EMAIL_BRAND.ink};">
              ...
              <p style="margin:0 0 16px 0; color:${EMAIL_BRAND.muted};">
                ${
                  safeCompany
                    ? `Účet pre firmu <strong style="color:${EMAIL_BRAND.ink};">${safeCompany}</strong> je aktívny.`
                    : "Váš veľkoobchodný účet je aktívny."
                }
                Odteraz sa môžete prihlásiť a využívať veľkoobchodné ceny a výhody.
              </p>
